...
use windjammer_runtime::collections::Vec;
...
pub fn from_args(mut args: Args) -> Result<Config, String> {
    ...
    let use_color = match args::color::as_str().as_str() {
        "always" => true,
        "never" => false,
...
impl ReplaceManager {
#[inline]
fn new(&self, mut dry_run: bool, mut backup: bool) -> Self {
        ReplaceManager { dry_run, backup }
}
#[inline]
fn replace_in_files(&self, mut matches: Vec<Match>, mut replacement: &str) -> Result<ReplaceStats, String> {
        let mut stats = ReplaceStats { files_modified: 0, total_replacements: 0, errors: vec![] };
        let mut files_to_replace = std::collections::HashMap::new();
        for m in matches {
            files_to_replace.entry(m.file.clone()).or_insert(vec![]).push(m);
        }
        for (file_path, file_matches) in files_to_replace {
            match self.replace_in_file(file_path.clone(), file_matches, replacement.clone()) {
                Ok(count) => {
                    stats.files_modified = stats.files_modified + 1;
                    stats.total_replacements = stats.total_replacements + count;
                },
                Err(e) => {
                    stats.errors.push(ReplaceError { file: file_path, error: e })
                },
            }
        }
        Ok(stats)
}
fn replace_in_file(&self, mut file_path: String, mut matches: Vec<Match>, mut replacement: String) -> Result<i64, String> {
        ...
        sorted_matches.sort_by(move |a, b| b.line_number.cmp(&a.line_number));
        let mut lines: Vec<String> = contents.lines().map(move |s| s.to_string()).collect();
        let mut replacement_count = 0;
        for m in sorted_matches {
            let line_idx = (m.line_number - 1) as usize;
            if line_idx >= lines.len() {
                log::warn("Line number out of bounds".to_string(), std::collections::HashMap::from([("file", file_path.clone()), ("line", m.line_number)]));
                continue;
            }
...
");
            ...
            log::info("File modified".to_string(), std::collections::HashMap::from([("file", file_path), ("replacements", replacement_count)]))
        } else {
            log::info("Dry run - would modify file".to_string(), std::collections::HashMap::from([("file", file_path), ("replacements", replacement_count)]))
        }
        Ok(replacement_count)
}
#[inline]
fn preview_replacements(&self, mut matches: Vec<Match>, mut replacement: &str) -> Vec<ReplacementPreview> {
        let mut previews: SmallVec<[_; 4]> = smallvec![];
        for m in matches {
            let new_text = m.line_text.replace(&m.match_text, &replacement);
            previews.push(ReplacementPreview { file: m.file, line_number: m.line_number, old_line: m.line_text, new_line: new_text, match_text: m.match_text, replacement: replacement.clone() });
        }
        previews
}
#[inline]
fn restore_from_backup(mut file_path: String) -> Result<(), String> {
        let backup_path = {
            let mut __s = String::with_capacity(64);
            write!(&mut __s, "{}.bak", file_path).unwrap();
            __s
        };
        if !fs::exists(backup_path.clone()) {
            return Err({
                let mut __s = String::with_capacity(64);
                write!(&mut __s, "Backup file not found: {}", backup_path).unwrap();
                __s
            });
        }
        fs::copy(backup_path, file_path.clone())?;
        log::info("File restored from backup".to_string(), std::collections::HashMap::from([("file", file_path)]));
        Ok(())
}
#[inline]
fn cleanup_backups(mut directory: String) -> Result<i64, String> {
        let mut count = 0;
        ...
        log::info("Backup files cleaned up".to_string(), std::collections::HashMap::from([("count", count)]));
        Ok(count)
}
}
