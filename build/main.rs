...
pub fn should_include_file(mut path: &str, mut config: &Config, mut gitignore_cache: &mut GitignoreCache) -> bool {
    ...
    if config.respect_ignore {
        let dir = path::parent(&Path::new(path)).and_then(move |p| p.to_str()).unwrap_or(".").to_string();
...
pub fn print_json(mut results: &SearchResults, mut duration: time::Duration) {
    ...
    println!("{}", json::stringify_pretty(&output).unwrap_or_else(move |e| e))
}
...
pub fn print_default(mut results: &SearchResults, mut config: &Config) {
    ...
    if config.use_color {
        println!("{}Found {} matches in {} files (searched {} files){}", COLOR_CYAN, results.total_matches, file_count, results.files_searched, COLOR_RESET)
    } else {
        println!("Found {} matches in {} files (searched {} files)", results.total_matches, file_count, results.files_searched)
    }
}
