...
#[inline]
fn find_match(mut line: &str, mut line_num: i64, mut file: &str, mut config: &Config) -> Option<Match> {
    let captures = config.pattern.captures(line)?;
    let match_obj = captures.get(0)?;
    let match_text = match_obj.as_str();
    let column = (match_obj.start() + 1) as i64;
...
#[inline]
fn find_all_matches(mut line: &str, mut line_num: i64, mut file: &str, mut config: &Config) -> Vec<Match> {
    let mut matches: SmallVec<[_; 4]> = smallvec![];
    for capture in config.pattern.captures_iter(line) {
        match capture.get(0) {
            Some(match_obj) => {
