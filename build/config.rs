...
#[inline]
fn from_args(mut args: Args) -> Result<Config, String> {
    let pattern_str = {
        if args.whole_word {
            let escaped = regex::escape(&args.pattern);
            format!("{}{}{}", "\\b", &escaped, "\\b")
...
#[inline]
fn get_file_extensions(mut file_type: String) -> Vec<String> {
    match file_type.as_str() {
        "rust" => vec!["rs"],
        "windjammer" | "wj" => vec!["wj"],
        "python" | "py" => vec!["py", "pyw"],
...
#[inline]
fn matches_file_type(mut path: &str, mut file_types: &[String]) -> bool {
    if file_types.is_empty() {
        return true;
    }
    let ext = std::path::extension(path).unwrap_or("");
...
#[inline]
fn should_exclude(mut path: &str, mut exclude_patterns: &[String]) -> bool {
    for pattern in exclude_patterns {
        if path.contains(pattern) {
            return true;
        }
