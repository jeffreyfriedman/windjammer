...
#[inline]
fn collect_files(mut paths: Vec<String>, mut config: &Config) -> Result<Vec<String>, String> {
    let mut all_files: SmallVec<[_; 4]> = smallvec![];
    let mut gitignore_cache = GitignoreCache::new();
    for path in paths {
        let files = walk_path(path, &config, &mut gitignore_cache)?;
...
fn walk_path(mut path: String, mut config: &Config, mut gitignore_cache: &mut GitignoreCache) -> Result<Vec<String>, String> {
    let mut files: SmallVec<[_; 4]> = smallvec![];
    if !fs::exists(&path) {
        return Err(format!("Path does not exist: {}", path));
    }
...
#[inline]
fn walk_dir(mut dir: String, mut config: &Config, mut files: &mut [String], mut gitignore_cache: &mut GitignoreCache) -> Result<(), String> {
    let entries = fs::read_dir(&dir)?;
    let gitignore_rules = {
        if config.respect_ignore {
            Some(gitignore_cache.get_rules(&dir))
...
fn should_include_file(mut path: &str, mut config: &Config, mut gitignore_cache: &mut GitignoreCache) -> bool {
    if !matches_file_type(path, &config.file_types) {
        return false;
    }
    if config.respect_ignore {
...
#[inline]
fn is_ignored(mut name: &str) -> bool {
    let ignored_dirs = vec!["target", "node_modules", ".git", ".svn", ".hg", "dist", "build", "__pycache__", ".cache", ".venv", "venv"];
    ignored_dirs.contains(&name.as_str())
}

#[inline]
fn is_likely_binary(mut path: &str) -> bool {
    let binary_extensions = vec!["exe", "dll", "so", "dylib", "a", "o", "png", "jpg", "jpeg", "gif", "bmp", "ico", "pdf", "zip", "ta", "gz", "bz2", "xz", "mp3", "mp4", "avi", "mov", "mkv", "wasm", "class", "pyc"];
    let ext = path::extension(&path).unwrap_or("");
    binary_extensions.contains(&ext)
}
