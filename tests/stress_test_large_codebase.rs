...
fn test_compile_deeply_nested() {
    ...
    println!("Generated {} lines of code with 50 levels of nesting", source.lines().count());
...
fn test_compile_10k_lines() {
    ...
    println!("Generated {} lines of code", source.lines().count());
    assert!(source.lines().count() >= 10_000, "Should generate 10K+ lines");
    ...
    if let Ok(program) = result {
        ...
        println!("Total compilation time for 10K+ lines: {:?}", start.elapsed());
        
        // Should handle 10K lines in reasonable time (< 30 seconds)
        assert!(start.elapsed().as_secs() < 30, "10K line compilation took too long");
    }
}
