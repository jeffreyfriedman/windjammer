...
fn test_scientific_notation_integer_base() {
    ...
    assert_eq!(tokens.len(), 1, "1e10 should be a single token, got: {:?}", tokens);
    match &tokens[0] {
        Token::FloatLiteral(f) => {
            assert!((f - 1e10).abs() < 1.0, "Expected 1e10 = 10000000000.0, got {}", f);
        }
...
fn test_scientific_notation_float_base() {
    ...
    assert_eq!(tokens.len(), 1, "2.5e3 should be a single token, got: {:?}", tokens);
    match &tokens[0] {
        Token::FloatLiteral(f) => {
...
fn test_scientific_notation_negative_exponent() {
    ...
    assert_eq!(tokens.len(), 1, "1e-5 should be a single token, got: {:?}", tokens);
    match &tokens[0] {
        Token::FloatLiteral(f) => {
...
fn test_scientific_notation_positive_exponent() {
    ...
    assert_eq!(tokens.len(), 1, "3E+2 should be a single token, got: {:?}", tokens);
    match &tokens[0] {
        Token::FloatLiteral(f) => {
...
fn test_scientific_notation_float_negative_exp() {
    ...
    match &tokens[0] {
        Token::FloatLiteral(f) => {
            assert!((f - 6.022e-23).abs() < 1e-30, "Expected 6.022e-23, got {}", f);
        }
        other => panic!("Expected FloatLiteral, got {:?}", other),
    }
}
...
fn test_scientific_notation_in_let_expression() {
    ...
    let float_tokens: Vec<&Token> = tokens.iter().filter(|t| matches!(t, Token::FloatLiteral(_))).collect();
    assert_eq!(float_tokens.len(), 1, "Should have exactly one float literal in 'let x = 1e10'");
}
...
"#);
    assert!(output.contains("1e10") || output.contains("10000000000"),
        "Generated Rust should contain the scientific notation value. Got:\n{}", output);
    assert!(!output.contains("e10;"), "Should NOT generate 'e10' as a separate identifier");
}
