...
| P1 | **Cross-module match-arm call to multi-use owned `string` formal must move not borrow** | `codegen_cross_module_match_arm_multi_use_owned_formal_gate_test` | ⚠️ RED — P3.194–P3.197 dogfood delegates `json + ""` once at public port (`parse_*_owned(json + "")`); re-probe still RED; inner body uses bare `owned` param |
