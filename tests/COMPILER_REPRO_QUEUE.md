...
| P1 | **WAL replay `replay_to_lsn` / path borrow cluster** | `cross_crate_dogfooding_ownership_test::dogfood_wal_replay_*` | 🔴 tip RED (2026-09-15) — `replay_to_lsn(self.path, …)` move/borrow; body `through` Copy; 12/51 dogfood filters failing |
| P1 | **`temp_path("recover")` cross-crate literal must stay `&str`** | `dogfood_temp_path_string_literal_no_to_string` | 🔴 tip RED (2026-09-15) — callee still `String` + `"recover".to_string()`; readonly text-return demotion not firing in multifile |
...
| P1 | **Cross-crate dogfooding ownership (51 filters)** | `cross_crate_dogfooding_ownership_test` | 🔴 **40/51 GREEN** (2026-09-15). Remaining 12: `temp_path` literal, WAL path/`through`, `MemoryEngine::put` borrow, forward-ref Key, store patch/enum layout, vec index compare, txn delegate, substrate enum layout |
