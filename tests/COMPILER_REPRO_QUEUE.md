...
| P1 | **Match arms yielding `string` must unify owned (`substring` vs binding)** | `codegen_match_string_arms_must_unify_gate_test` | ⚠️ RED on tip — P3.192 dogfood keeps `dims_raw.substring(0, end + 1) + ""` / `dims_raw + ""` in `parse_analytics_schema_fields` until gate GREEN |
