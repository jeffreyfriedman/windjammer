...
| P1 | **Cross-module match-arm call to multi-use owned `string` formal must move not borrow** | `codegen_cross_module_match_arm_multi_use_owned_formal_gate_test` | ⚠️ RED — P3.199 re-probe: drop `parse_*_owned(json)` at public port → `read_models` E0308; keep `json + ""` delegate until tip greens gate |
