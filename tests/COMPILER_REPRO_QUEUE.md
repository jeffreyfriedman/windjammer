...
| P1 | **Match arms yielding `string` must unify owned (`substring` vs binding)** | `codegen_match_string_arms_must_unify_gate_test` | ⚠️ RED inline — P3.193 dogfood uses `clip_json_array_through_bracket` helper (tip **GREEN** for extracted fn); inline match in callee still RED |
| P1 | **Cross-module match-arm call to multi-use owned `string` formal must move not borrow** | `codegen_cross_module_match_arm_multi_use_owned_formal_gate_test` | ⚠️ RED — P3.194–P3.198 dogfood delegates `json + ""` once at public port (`parse_*_owned(json + "")`); PanelHead sweep complete; drop probe still RED |
...
| P1 | **WDB-110: isolate-transpile `.clone()` into owned `string` formal must not emit `&path.clone()`** | `wdb110_same_file_owned_string_clone_call_site_cargo_checks`, `wdb110_tip_isolate_owned_string_clone_must_not_borrow_at_call_site` | ⚠️ RED — WindjammerDB dogfood strips `&` on `cli_run_parquet_load(li_path.clone(), …)` after per-file transpile; retire `dogfood_gen_p153.py` patch when green |
