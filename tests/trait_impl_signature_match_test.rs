...
fn compile_rust(code: &str) -> (bool, String) {
    ...
    let output = Command::new("rustc")
        .args(&["--crate-type", "lib", test_file, "-o", "/tmp/trait_impl_test_rust.rlib", "--edition", "2021"])
        .output()
...
"#;
    ...
    assert!(generated.contains("trait GameLoop"), "Should have trait definition");
    ...
    assert!(generated.contains("impl GameLoop for MyGame"), "Should have impl");
    ...
    assert!(impl_section.contains("fn update(&mut self, delta: f32"), 
    ...
    assert!(rust_success, "Generated Rust should compile without E0053 errors. Errors:\n{}", rust_stderr);
    assert!(!rust_stderr.contains("E0053"), "Should not have E0053 (incompatible type for trait). Stderr:\n{}", rust_stderr);
}
...
"#;
    ...
    assert!(trait_section.contains("fn update(&mut self, input: Input)"), 
        "Trait should have 'input: Input'. Trait:\n{}", trait_section);
    assert!(trait_section.contains("fn render(&self, ctx: RenderContext)"), 
    ...
    assert!(impl_section.contains("fn update(&mut self, input: Input"), 
        "Impl should match trait with 'input: Input'. Impl:\n{}", impl_section);
    assert!(impl_section.contains("fn render(&self, ctx: RenderContext"), 
        "Impl should match trait with 'ctx: RenderContext'. Impl:\n{}", impl_section);
    
    // Verify Rust compilation succeeds
    let (rust_success, rust_stderr) = compile_rust(&generated);
    assert!(rust_success, "Generated Rust should compile. Errors:\n{}", rust_stderr);
