...
fn test_shader_directory_excluded_from_discovery() {
    ...
    fs::write(
        ...
        "@vertex pub fn vs() -> vec4<float> { vec4(0.0, 0.0, 0.0, 1.0) }",
    )
    .unwrap();
    ...
    assert!(
        file_names.contains(&"vector.wj".to_string()),
        "Regular .wj files should be discovered"
    );
    assert!(
        !file_names.contains(&"vertex.wj".to_string()),
...
fn find_wj_files_recursive_excluding_shaders(
    dir: &Path,
