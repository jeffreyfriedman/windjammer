...
fn test_static_method_inference() {
    ...
    let output = Command::new("cargo")
        .args(&["run", "--release", "--bin", "wj", "--", 
                ...
                "--output", "/tmp/wj_static_method_test",
                "--no-cargo"])
        .output()
    ...
    let generated_code = std::fs::read_to_string("/tmp/wj_static_method_test/static_method_inference_test.rs")
    ...
    let rustc_output = Command::new("rustc")
        .args(&[
            ...
            "--crate-type", "lib",
            "--out-dir", "/tmp/wj_static_method_test",
        ])
        .output()
