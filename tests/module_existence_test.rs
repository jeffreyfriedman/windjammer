...
fn test_module_file_skips_missing_modules() {
    ...
    assert!(
        ...
        "button.rs exists, should be declared"
    );
    assert!(
        mod_rs.contains("pub mod input;"),
        "input.rs exists, should be declared"
    );

    // Verify mod.rs itself is NOT declared (would cause recursion)
    assert!(
        !mod_rs.contains("pub mod mod;"),
        "mod.rs should NOT be declared"
    );

    // Verify lib.rs is NOT declared if it doesn't exist
    assert!(
        !mod_rs.contains("pub mod lib;"),
        "lib.rs doesn't exist, should NOT be declared"
    );
}
...
fn test_nested_modules_only_declare_existing_files() {
    ...
    assert!(
        root_mod.contains("pub mod components;"),
        "components subdir should be declared"
    );
    ...
    assert!(
        components_mod.contains("pub mod button;"),
        "button should be declared"
    );
    assert!(
        components_mod.contains("pub mod input;"),
        "input should be declared"
    );
    assert!(
        !components_mod.contains("pub mod mod;"),
        "mod.rs should NOT declare itself"
    );
}
...
fn test_cargo_toml_excluded_from_module_list() {
    ...
    assert!(
        !mod_rs.contains("Cargo"),
        "Cargo.toml should not appear in mod.rs"
    );
    assert!(
        !mod_rs.contains("toml"),
        "Cargo.toml should not appear in mod.rs"
    );
