...
fn test_detect_duplicate_struct_exports() {
    ...
    fs::write(src_dir.join("module_a.wj"), r#"
pub struct Config {
    value: int,
}
"#).unwrap();
    
    fs::write(src_dir.join("module_b.wj"), r#"
pub struct Config {
    setting: string,
}
"#).unwrap();
    ...
    let has_ambiguous = mod_rs.contains("pub use module_a::*;") 
        && mod_rs.contains("pub use module_b::*;");
...
fn test_explicit_reexports_for_conflicts() {
    ...
    // Both export "Error"
    fs::write(src_dir.join("http.wj"), r#"
pub struct Request {}
pub struct Error {
    code: int,
}
...
fn test_warning_for_potential_conflicts() {
    ...
    let uses_glob_reexports = mod_rs.contains("pub use types::*;") 
        && mod_rs.contains("pub use errors::*;");
