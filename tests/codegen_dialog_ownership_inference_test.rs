...
"#;
    ...
    // Check if & was added to item_id
    let has_correct_call = generated_code.contains("game_state.inventory.has_item(&item_id, qty)");
    let has_wrong_call = generated_code.contains("game_state.inventory.has_item(item_id, qty)")
        && !generated_code.contains("game_state.inventory.has_item(&item_id, qty)");

    if has_wrong_call {
        eprintln!("ERROR: String → &str conversion not applied!");
        eprintln!("Expected: game_state.inventory.has_item(&item_id, qty)");
        eprintln!("Found: game_state.inventory.has_item(item_id, qty)");
    }
