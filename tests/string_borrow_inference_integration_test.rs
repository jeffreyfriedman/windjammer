...
fn test_string_borrow_inference() {
    ...
    if !output.status.success() {
        eprintln!("Compiler stderr: {}", String::from_utf8_lossy(&output.stderr));
        eprintln!("Compiler stdout: {}", String::from_utf8_lossy(&output.stdout));
    ...
    // Verify log_message infers &str (read-only parameter)
    assert!(
        generated_code.contains("pub fn log_message(self, message: &str)"),
        "log_message should infer message: &str (parameter is only read)"
    );
