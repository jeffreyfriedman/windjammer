...
//! P3.313: `for zi in 0..(zd + 1)` with `zd: i32` must not emit `zd as i64 + 1_i32`.
//!
//! Product: `mesh_primitives.wj` → E0277 / E0308 on range end expression.
//! (P3.312 is auto_mut MutBorrowed Vec; P3.311 is dotenv vec-index — do not collide.)
...
fn i32_range_end_add_must_not_split_i64_i32() {
    ...
    let map = test.compile().expect("P3.313 compile");
    ...
    assert!(
        ...
        "RED P3.313: i32 range end add must stay i32 width, not i64 + i32 split:\n{rs}"
    );
    test.cargo_check().expect("P3.313 cargo-check");
}
