...
fn test_no_shorthand_when_type_conversion_needed() {
    ...
    assert!(generated_code.contains("name: name.to_string()") || generated_code.contains("name: (&name).to_string()"), 
        "Should convert &str to String in struct field\nGenerated:\n{}", generated_code);
    assert!(!generated_code.contains("User { name }") && !generated_code.contains("User{name}"), 
