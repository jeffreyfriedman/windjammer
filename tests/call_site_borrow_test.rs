...
fn test_borrowed_reference_param_adds_ampersand() {
    ...
    let decision = should_borrow_at_call_site(&sig, 0, &arg, "quest_id", "is_quest_active", false, None);
...
fn test_copy_scalar_i32_no_ampersand() {
    ...
    let decision = should_borrow_at_call_site(&sig, 0, &arg, "64", "push", false, Some("Vec"));
...
fn test_string_literal_to_str_param_no_extra_ampersand() {
    ...
    let decision = should_borrow_at_call_site(&sig, 0, &arg, "\"hello\"", "log", false, None);
    assert!(
        !decision.add_ref,
