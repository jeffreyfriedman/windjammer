...
## Build-Time Security Validation (Catch Issues Early)
...
### Solution: Integrated Security Validation in Build

**No external dependencies - built into `wj build`:**
...
**When validation fails (blocks build by default):**
...
This blocks the build to prevent insecure deployment.

Options:
  1. Remove package: wj remove suspicious-lib
  2. Review first: wj show suspicious-lib@1.0.0
  3. Override for CI testing: wj build --allow-violations
     (logs violation, requires approval later)

Recommendation: Review before deploying

Build blocked (security policy)
```

**CI mode (warn but don't block):**

```bash
# For CI environments where you want warning, not blocking
wj build --ci-mode
...
⚠️  Security warning: 1 issue found

Package: new-package@0.1.0
Issue: Low trust score (3.2/10)
Risk: MEDIUM (needs review)

This is logged but doesn't block CI.
Post-build review required: wj security review-violations

✅ Build complete (warnings logged)

# CI can decide policy
exit 0  # Allow with warnings
# OR
exit 1  # Fail on any warning
```
...
# wj.toml
[security.build]
# Block build on violations (default: true)
block_on_violation = true

# Warn but don't block (useful for CI)
warn_only = false

# Skip security checks entirely (NOT RECOMMENDED)
skip_security = false

# Fast mode (cached results, for iteration)
fast_mode = true  # Use cached analysis when possible
```

**Benefits:**
- VCS-agnostic (works with git, hg, svn, p4, or no VCS)
- Built into build process (no external hooks)
- Fast feedback (1-3 seconds, part of normal build)
- Configurable (block, warn, or skip)
- CI-friendly (deterministic, loggable)
...
### Capability Diff (Enhanced PR Analysis)

**Capability-only diff for focused reviews:**

```bash
wj security diff main..feature-branch --capabilities-only

Capability Diff: main → feature-branch
...
+ fs_write:./exports/*
  ...
  └─> Files changed: src/export.wj (+45 lines)
  └─> Risk: ✅ LOW (controlled path)
  └─> Justification: User export feature (#1234)

+ net_egress:api.stripe.com
  └─> Package: payment-lib@1.0.0 (new dependency)
  └─> Files changed: Cargo.toml (+1 dependency)
  └─> Risk: ⚠️ MEDIUM (third-party API)
  └─> Alternative: stripe-official@3.0.0 (higher trust 9.2/10)

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
CAPABILITY BUDGET
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

Before: 5 capabilities
After: 7 capabilities (+40%)
Budget: 10 capabilities (70% used)

⚠️  Warning: Approaching budget limit
Recommendation: Consider refactoring to reduce I/O operations
```

### Executive Dashboard

**Simplified metrics for leadership:**

```bash
wj dashboard --executive

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🛡️  Windjammer Security: Executive Summary
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

Security Status: ✅ EXCELLENT (9.2/10)
  └─> Industry average: 6.5/10 (top 5%)

Key Metrics:
  ✅ Zero critical vulnerabilities (30 days)
  ✅ 14 attacks prevented ($700K impact)
  ✅ 4.3/5 developer satisfaction
  ✅ All compliance passing (SOC 2, ISO, PCI)

ROI: 299x
  └─> Cost: $2,801/year
  └─> Return: $840,000/year (incidents prevented)
  └─> Payback: 1.2 days
...
Bottom Line: Security working, developers happy, ROI positive

Export: wj dashboard --executive --export summary.pdf
```

---

## Secure Container Generation (Capability-Driven Dockerfiles)

### The Problem: Docker Security is Complex and Error-Prone

**Manual Dockerfile (typical):**
```dockerfile
FROM ubuntu:latest           # ❌ Latest tag (not reproducible)
RUN apt-get update && \      # ❌ Running as root
    apt-get install -y ...   
COPY . /app                  # ❌ Everything copied (secrets?)
USER root                    # ❌ Root user in production!
EXPOSE 8080
CMD ["./app"]
```
...
- Wrong Linux capabilities
- No read-only filesystem
- Secrets in image layers
- Non-reproducible builds

**Developer:** "I don't know how to configure all this securely!"

### Solution: Automatic Minimal-Privilege Container Generation

**Windjammer analyzes your app's capabilities and generates secure container config automatically.**

```bash
wj container generate

Analyzing application capabilities...

App: my-api
Capabilities detected:
  ├─> net_egress (HTTP server)
  ├─> fs_read:./config/* (config files)
  ├─> fs_write:./logs/* (logging)
  └─> No privileged operations

Generating secure container configuration...

✅ Dockerfile generated
✅ Seccomp profile generated
✅ AppArmor profile generated (optional)
✅ Kubernetes security context generated

Files created:
  ├─> Dockerfile (minimal privilege)
  ├─> .dockerignore (exclude secrets)
  ├─> seccomp-profile.json
  ├─> apparmor-profile.txt
  └─> k8s-security-context.yaml

Build: docker build -t my-api:latest .
Run: docker run --security-opt seccomp=seccomp-profile.json my-api:latest
```

#### Generated Dockerfile (Capability-Driven)

```dockerfile
# Generated by Windjammer Security Framework
# Based on application capability analysis
# Generated: 2026-03-21T16:00:00Z

# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# Stage 1: Build (multi-stage for minimal final image)
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
COPY --chown=appuser:appuser Cargo.toml Cargo.lock wj.toml ./
...
# Build application (release mode, stripped)
RUN wj build --release && \
    strip target/release/my-api
...
LABEL org.opencontainers.image.created="2026-03-21T16:00:00Z"
LABEL org.opencontainers.image.source="https://github.com/company/my-api"
LABEL security.capabilities="net_egress,fs_read,fs_write"
LABEL security.generated-by="windjammer-security"

# Create directory structure with minimal permissions
# Detected capabilities: fs_read:./config/*, fs_write:./logs/*
WORKDIR /app

# Copy binary from builder (single file, minimal)
COPY --from=builder --chown=nonroot:nonroot /build/target/release/my-api /app/my-api

# Copy ONLY required files (based on fs_read capability analysis)
COPY --from=builder --chown=nonroot:nonroot /build/config /app/config

# Create writable directories (based on fs_write capability analysis)
# Note: Volumes should be mounted here in production
USER root
RUN mkdir -p /app/logs && \
    chown nonroot:nonroot /app/logs && \
    chmod 0755 /app/logs
USER nonroot

# Expose port (detected from net_egress capability)
EXPOSE 8080

# Health check (best practice)
HEALTHCHECK --interval=30s --timeout=3s --start-period=5s --retries=3 \
  CMD ["/app/my-api", "--healthcheck"]

# Run as non-root user (UID 65532)
USER nonroot:nonroot

# Read-only root filesystem (security best practice)
# Writable volumes must be explicitly mounted
# Use: docker run --read-only -v logs:/app/logs my-api

# Environment (minimal, non-secret only)
...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# Security Configuration (see seccomp-profile.json)
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
#
# Run with:
#   docker run --rm \
...
#     my-api:latest
#
# Kubernetes deployment: See k8s-security-context.yaml
```

#### Generated Seccomp Profile (Minimal Syscalls)

```json
{
  "comment": "Generated by Windjammer Security Framework",
  "generated": "2026-03-21T16:00:00Z",
  ...
  "comment_syscalls": "Minimal syscalls based on detected capabilities (net_egress, fs_read, fs_write)"
}
```

#### Generated Kubernetes SecurityContext

```yaml
# Generated by Windjammer Security Framework
# Based on application capability analysis: net_egress, fs_read, fs_write
...
metadata:
  ...
  labels:
    app: my-api
    security.windjammer/generated: "true"
spec:
  securityContext:
    # Pod-level security
    runAsNonRoot: true
    ...
    seccompProfile:
      type: Localhost
      localhostProfile: profiles/windjammer-my-api.json
  ...
  - name: my-api
    ...
    securityContext:
      ...
      capabilities:
        ...
        add:
          - NET_BIND_SERVICE  # Only add required capability (for port 8080)
    ...
    - name: config
      ...
      readOnly: true  # fs_read capability → read-only mount
    - name: logs
      mountPath: /app/logs
      readOnly: false  # fs_write capability → writable mount
  ...
  - name: config
    configMap:
      name: my-api-config
  - name: logs
    emptyDir: {}  # Ephemeral logs (use persistent volume in production)
```

#### Capability → Linux Capability Mapping

**Windjammer automatically maps its capabilities to Linux capabilities:**

| Windjammer Capability | Linux Capability | Syscalls Allowed |
|----------------------|------------------|------------------|
| `net_egress` | `CAP_NET_BIND_SERVICE` | socket, bind, listen, accept, connect, send, recv |
| `net_ingress` | `CAP_NET_BIND_SERVICE` | socket, bind, listen, accept |
| `fs_read` | (none) | open, openat, read, readv, stat |
| `fs_write` | (none) | open, openat, write, writev |
| `fs_read:privileged` | `CAP_DAC_READ_SEARCH` | open (any file), read (any file) |
| `fs_write:privileged` | `CAP_DAC_OVERRIDE` | open (any file), write (any file) |
| `process_spawn` | `CAP_SYS_ADMIN` | fork, execve, clone |
| `sys_admin` | `CAP_SYS_ADMIN` | (all privileged syscalls) |

**Example: Web API (net_egress + fs_read + fs_write)**
```dockerfile
# Linux capabilities: NET_BIND_SERVICE only
# Seccomp: socket, bind, listen, accept, read, write (35 syscalls)
# User: nonroot (65532)
# Filesystem: Read-only except /app/logs
```

#### Interactive Container Configuration

```bash
wj container configure --interactive

Windjammer Container Security Configurator

Analyzing your application...
  ✅ Detected capabilities: net_egress, fs_read, fs_write

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Base Image Selection
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

Recommended base images (by security):

  1. ✅ distroless/static:nonroot (BEST)
     └─> Size: 2 MB
     └─> Attack surface: MINIMAL (no shell, no package manager)
     └─> Security: EXCELLENT (static binary only)
     
  2. ⚠️  alpine:latest
     └─> Size: 5 MB
     └─> Attack surface: LOW (minimal packages)
     └─> Security: GOOD (but has shell and apk)
     
  3. ⚠️  debian:slim
     └─> Size: 80 MB
     └─> Attack surface: MEDIUM (full OS)
     └─> Security: FAIR (larger attack surface)

Choose: (1-3) 1

✅ Using distroless/static:nonroot

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Security Hardening
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

Security features (all recommended ✅):

  [x] Run as non-root user (UID 65532)
  [x] Read-only root filesystem
  [x] Drop all Linux capabilities (add back only required)
  [x] Seccomp profile (minimal syscalls)
  [x] No new privileges
  [x] AppArmor profile (optional)
...
Detected filesystem access:

Read-only (fs_read):
  ├─> ./config/*.toml (config files)
  └─> Recommendation: Mount as ConfigMap (Kubernetes)

Read-write (fs_write):
  ├─> ./logs/*.log (log files)
  └─> Recommendation: Mount as emptyDir or persistent volume

Exclude from image:
  ⚠️  ./secrets/ (contains sensitive data)
...
Detected network capability: net_egress

Listening port: 8080 (detected)
Protocol: HTTP (assumed)

Linux capability: CAP_NET_BIND_SERVICE required

Health check:
  Endpoint: /health (auto-detected)
  Interval: 30s
  Timeout: 3s
...
Files generated:
  ...
  ✅ seccomp-profile.json (35 syscalls allowed)
  ✅ apparmor-profile.txt (optional)
  ✅ k8s-security-context.yaml (production-ready)
...
Next steps:
  ...
  2. Build: docker build -t my-api:latest .
  3. Test locally: docker-compose up
  4. Deploy: kubectl apply -f k8s-security-context.yaml

Documentation: ./container-security-README.md
```

#### Testing Container Security

```bash
# Validate generated container security
wj container test
...
✅ Minimal syscalls (35 allowed, 300+ blocked)
✅ No secrets in image layers
✅ Distroless base image (2 MB)
✅ Health check configured
⚠️  No resource limits (add in k8s-security-context.yaml)

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Runtime Tests (with seccomp)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

Running container with security profile...
...
✅ Can write logs
❌ BLOCKED: Attempt to read /etc/passwd (correct!)
❌ BLOCKED: Attempt to spawn shell (correct!)
❌ BLOCKED: Attempt to open privileged port 80 (correct!)

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Vulnerability Scan (Trivy)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

Scanning image: my-api:latest
...
Image size: 12 MB (excellent)
Base image: distroless/static:nonroot (no vulnerabilities)
...
Passed: 11/12 tests
Warnings: 1 (add resource limits)
...
**Automatic minimal-privilege configuration:**
- Analyzes app capabilities → generates exact container permissions
- No manual seccomp/AppArmor configuration
- No guessing at Linux capabilities
...
**Zero security expertise needed:**
- Developer: "I need a container"
- Windjammer: "Here's a secure one based on your code"
- No security knowledge required

**Integration with ecosystems:**
- Docker (standard Dockerfile)
- Kubernetes (SecurityContext, PodSecurityPolicy)
- Cloud (ECS, Cloud Run, App Engine)
- CI/CD (automated security scanning)

---

## Quick Security Toggle (Prototyping Mode)

### The Problem: Security Friction During Rapid Prototyping

**Scenario:**
```
Developer: "I'm just prototyping, I don't want security prompts!"
(Edits wj.toml, disables security, forgets to re-enable, ships to prod...)
```

### Solution: Time-Limited Security Disable

**Quick toggle with automatic re-enable:**

```bash
# During prototyping
wj security disable --reason "rapid prototyping" --until tomorrow

Security disabled (24 hours)
  └─> Reason: rapid prototyping
  └─> Re-enables: 2026-03-22 16:00:00
  └─> Logged to: .wj-security-overrides.log

Warnings:
  ⚠️  All security checks BYPASSED
  ⚠️  Not for production use
  ⚠️  Review dependencies before shipping

Quick re-enable: wj security enable

# Next day, auto re-enables
wj build

🛡️  Security re-enabled (time limit reached)

Added while disabled: 3 new packages

Review? (Y/n) y

Reviewing packages added during disabled period:

  1. experimental-lib@0.1.0
     └─> Trust score: 2.1/10 (VERY LOW)
     └─> Capabilities: sys_admin (DANGEROUS)
     └─> Action: Remove before production

  2. prototype-helper@1.0.0
     └─> Trust score: 4.5/10 (LOW)
  ...
  3. debug-tool@2.0.0
     └─> Trust score: 8.7/10 (HIGH)
     └─> Capabilities: logic_only
     └─> Action: OK for production ✅

Recommendation: Remove #1 and #2 before shipping
...
- `--until 4pm` (until specific time today)
- `--for 2h` (2 hour duration)
- `--until friday` (until end of week)

**Safety:**
- Auto re-enables (can't forget)
- Full audit trail (logged)
- Review on re-enable (catch issues)
- Can't disable in CI (requires TTY)

**Benefits:**
- Unblocks rapid prototyping
- Safety: time-limited, auto-review
- Audit trail for compliance
- Can't be abused (requires explicit reason)

---

## Compiler Version Pinning (Team Consistency)

### The Problem: Compiler Version Drift Across Team

**Scenario:**
```
Alice: wj 0.46.0 (everything works)
Bob: wj 0.47.0 (installed latest, behavior differs)
CI: wj 0.45.0 (outdated, fails differently)
```

### Solution: Pin Compiler Version in Lock File
...
lock_version = 1
compiler_version = "0.46.0"  # Team uses this version
compiler_hash = "sha256:abc123..."
...
⚠️  Compiler version mismatch

Project requires: 0.46.0
You have: 0.47.0 (newer)

Behavior may differ (features/bugs changed between versions)

Options:
  1. Install pinned version: wj install 0.46.0
     └─> Recommended for team consistency
  
  2. Use installed version: wj build --ignore-compiler-version
     └─> Risk: May break lock file assumptions
  
  3. Update project: wj lock update-compiler-version
     └─> Updates lock file to 0.47.0 (team must upgrade)

Automatically install 0.46.0? (Y/n) y

Downloading wj 0.46.0...
✅ Installed to: ~/.wj/versions/0.46.0/
✅ Using pinned version

Building with wj 0.46.0...
```
...
Installed Windjammer versions:
  ├─> 0.46.0 (pinned by 3 projects)
  ├─> 0.47.0 (current, used by 1 project)
  └─> 0.45.0 (old, used by 0 projects)

Space used: 124 MB

Clean old versions: wj versions clean

# Update lock file to newer version
wj lock update-compiler-version --to 0.47.0

Updating compiler version...

Checking compatibility:
  ✅ All features supported in 0.47.0
  ⚠️  1 deprecation warning (see CHANGELOG)
  ✅ No breaking changes

Updated: .wj-capabilities.lock
  └─> compiler_version: 0.46.0 → 0.47.0

Commit this change for team to upgrade.
```
...
- name: Install Windjammer
  run: wj install --from-lock
  # Automatically installs version from lock file
```

**Benefits:**
- Team uses same compiler version
- CI uses same version as local
- Explicit version upgrades (not accidental)
- Multiple versions can coexist (~40 MB each)

---

## Migration from Existing Security Tools

### The Problem: Teams Already Using Snyk/Dependabot/etc.

**Barrier to adoption**: "We already have Snyk, why switch?"

### Solution: Migration Guides with Side-by-Side Comparison

**Automatic migration from Snyk:**

```bash
wj migrate-from snyk

Analyzing Snyk configuration...

Found Snyk setup:
  ├─> .snyk policy file (23 rules)
  ├─> snyk.yml (CI configuration)
  ├─> 12 ignored vulnerabilities
  └─> 3 custom severity thresholds

Generating Windjammer equivalent...

✅ Converted .snyk → wj-security-policy.rego (27 rules)
✅ Converted snyk.yml → .github/workflows/wj-security.yml
✅ Migrated ignored vulns → wj.toml [security.exceptions]
✅ Migrated severity levels → wj.toml [security.thresholds]
...
Snyk (before):
  ├─> Scan time: 5 minutes (CI)
  ├─> False positives: 12/month
  ├─> Policy updates: Manual (via .snyk file)
  ├─> Cost: $99/developer/month
  └─> Integration: Separate tool

Windjammer (after):
  ├─> Scan time: 1 second (build-time, cached)
  ├─> False positives: 2/month (learning system)
  ├─> Policy updates: Automatic (via code analysis)
  ├─> Cost: $0 (built into compiler)
  └─> Integration: Native (no separate tool)

Improvements:
  ⏱️  300x faster (5 min → 1 sec)
  🎯 6x fewer false positives
  💰 $1,188/developer/year savings
...
Phase 1: Parallel (Week 1-4)
  ├─> Keep Snyk running
  ├─> Add Windjammer alongside
  ├─> Compare results
  └─> Gain confidence

Phase 2: Primary (Week 5-8)
  ├─> Windjammer primary
  ├─> Snyk backup only
...
Phase 3: Full Migration (Week 9+)
  ├─> Remove Snyk
  ├─> Commit to Windjammer
  ├─> Update team docs
  └─> Cancel Snyk subscription

Rollback: Keep .snyk for 90 days (safety)

Next steps:
  1. Review generated files (git diff)
  2. Test build: wj build
  3. Commit changes: git commit -m "Migrate from Snyk to Windjammer"
  4. Enable parallel run in CI

Questions? wj migrate-from snyk --help
```

**Other supported migrations:**

```bash
wj migrate-from dependabot    # GitHub Dependabot
wj migrate-from renovate      # Renovate Bot
wj migrate-from trivy         # Aqua Trivy
wj migrate-from grype         # Anchore Grype
```

**Migration guide output:**

```markdown
# Migration from Snyk to Windjammer

## Overview

This project has been migrated from Snyk to Windjammer's built-in security.

## What Changed

### Configuration Files
- **Removed**: `.snyk` (policy file)
- **Added**: `wj-security-policy.rego` (converted policy)
- **Modified**: `.github/workflows/` (CI configuration)

### Security Scanning
- **Before**: Snyk runs in CI (5 minute scans)
- **After**: Windjammer runs at build time (1 second scans)

### Ignored Vulnerabilities
All 12 ignored vulnerabilities from `.snyk` have been migrated to:
```toml
[security.exceptions]
"CVE-2024-1234" = "False positive (not exploitable)"
"CVE-2024-5678" = "Accepted risk (low severity)"
```

### Custom Policies
All 23 custom rules from `.snyk` have been converted to Rego policies.
...
wj migrate-to snyk --rollback
```

Snyk configuration backed up to: `.snyk.backup`

## Team Training

- Documentation: ./SECURITY.md
- Quick start: wj security --help
- Dashboard: wj security dashboard
...
**Benefits:**
- Automated migration (not manual)
- Side-by-side comparison (see value)
- Parallel run (risk-free testing)
- Rollback plan (safety net)
- Clear migration path (reduces friction)
