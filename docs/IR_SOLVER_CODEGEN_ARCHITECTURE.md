...
- **Reconcile shared-borrow reapply is IR-only (2026-08-24):** fresher-sig / WDB-101 path uses `enforce_ownership_contract_on_coerced_arg` (with collection-key `expected=Ref`). `should_borrow_at_call_site*` deleted; IR coercion gates live in `tests/ir_call_site_coercion_oracle_test.rs`.
...
- **WDB dogfood gates (IR-only, 2026-08-29):** WDB-099/101/107 ownership gates run against tip `call_sites` IR only. PRE snapshot worktree retired — see `codegen_owned_reuse_after_helper_test` for WDB-100.
...
- **`ir/formal_predicates.rs` (2026-08-24):** IR-neutral text/ref formal predicates; codegen delegates (`types`, `string_utilities`, `call_signature_resolution`). `emission_contract` uses predicates + `type_analysis_pure` (only `emitted_owned_arg_contract` remains codegen-linked).
