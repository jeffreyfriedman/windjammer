...
1. ✅ **Rust API compatibility** - Use `% 100 == 0` instead of `is_multiple_of()`
2. ✅ **File I/O race condition** - `sync_all()` + `flush()` + 10ms sleep on ALL platforms
3. ✅ **Windows stack overflow** - `RUST_MIN_STACK=16MB` for all threads
...
// Aggressive sync: flush + sync_all + explicit drop + sleep
{
    use std::io::Write;
    ...
    file.sync_all()?;    // Force OS to write to disk
    drop(file);          // Close handle immediately
}
std::thread::sleep(std::time::Duration::from_millis(10));  // Let FS complete
```

### Stack Size (All Threads)
```toml
# .cargo/config.toml
[env]
RUST_MIN_STACK = "16777216"  # 16MB for ALL threads (main + test + worker)

[target.x86_64-pc-windows-msvc]
rustflags = ["-C", "link-arg=/STACK:16777216"]  # Main binary
```
