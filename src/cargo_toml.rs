...
pub fn generate_wasm_cargo_toml(output_dir: &Path, source_dir: &Path) -> Result<()> {
    ...
    let package_name = resolve_package_name_with_existing_cargo(output_dir, &format!("{project_snake}_wasm"));
...
fn propagate_source_cargo_deps(source_dir: &Path, existing_deps: &[String]) -> Vec<String> {
    ...
    let skip_crates: HashSet<&str> = [
        "windjammer", "windjammer-runtime", "windjammer_runtime",
    ].into_iter().collect();

    let candidates = [
        source_dir.join("Cargo.toml"),
        source_dir.parent().map(|p| p.join("Cargo.toml")).unwrap_or_default(),
    ];
    ...
    for line in content.lines() {
        ...
        let already_present = existing_deps.iter().any(|d| {
            d.starts_with(dep_name) || d.starts_with(&dep_name.replace('-', "_"))
        });
...
fn detect_external_crate_deps(output_dir: &Path, source_dir: &Path) -> Vec<String> {
    ...
    let builtin_crates: HashSet<&str> = [
        ...
        "windjammer_runtime", "windjammer", "serde", "serde_core", "smallvec",
        "glob", "typenum", "bytemuck",
    ].into_iter().collect();
...
fn try_find_crate_at(root: &Path, hyphenated: &str, crate_name: &str, output_dir: &Path) -> Option<String> {
...
fn check_cargo_toml(crate_dir: &Path, crate_name: &str, output_dir: &Path) -> Option<String> {
    ...
    let abs = crate_dir.canonicalize().unwrap_or_else(|_| crate_dir.to_path_buf());
    let abs_output = output_dir.canonicalize().unwrap_or_else(|_| output_dir.to_path_buf());
    if abs == abs_output {
        return None;
    }
