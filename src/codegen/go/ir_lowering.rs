//! Go backend IR integration — runs Analyzer + IrPipeline for signature-driven lowering.
...
/// Run shared IR analysis for Go codegen. Errors are non-fatal (parse-only fallback).
pub fn prepare_ir_context<'ast>(program: &Program<'ast>) -> Option<IrContext<'ast>> {
    analyze_and_lower(program).ok()
}

/// Encode a call argument for Go using IR coercion rules.
pub fn encode_go_call_argument(
    ctx: &IrContext<'_>,
    callee_name: &str,
    arg_index: usize,
    arg_str: &str,
