            ...
            .map(|(i, (_label, arg))| {
                ...
                if self.ir_cutover.call_sites {
                    ...
                    if let Some(mut coerced) = self.apply_ir_call_site_coercion(
                        ...
                        let resolved_sig = receiver_rt.as_deref().and_then(|rt| {
                            self.resolve_method_function_signature(rt, method, arguments.len())
                        });
                        let wants_ref = {
                            let expects_shared = resolved_sig
                                .as_ref()
                                .map(|sig| {
                        ...
                        let wants_owned = {
                            ...
                            }) || contract(&fallback_sig, fallback_pidx)
                        };
                        if let Expression::Identifier { name, .. } = arg_to_generate {
                            ...
                            if receiver_is_self && (caller_owned_param || is_mixed_forwarder) {
                                ...
                                if mixed_forward_ref
                                    || (matches!(
                                        ...
                                        // Owned Copy aggregates (`other: Lsn`) must not gain
                                        // `&through` from stale IR Ref expectations (regression-060).
                                        && !wants_owned)
                                {
                                    ...
                                    && !coerced.starts_with("&mut ")
                                {
                                    ...
                                    let copy_aggregate = self.current_function_params.iter().any(
                                        |p| {
                                            p.name == *name
                        ...
                        if let Expression::Identifier { name, .. } = arg_to_generate {
                            if self.match_arm_bindings.contains(name.as_str()) {
                                let callee_borrows = receiver_rt.as_deref().and_then(|rt| {
                        ...
                        if !coerced.starts_with('&') {
                            if let Some(rt) = receiver_rt.as_deref() {
                                    ...
                                    )
                                {
                                    ...
                                    if self.ir_sig_arg_expects_shared_borrow(&reg_sig, i)
                                        && !crate::codegen::rust::signature_promotion::emitted_owned_arg_contract(
                                            &reg_sig, pidx,
                            ...
                            && !coerced.starts_with("&mut ")
                        {
                            ...
                            && self.ir_sig_arg_expects_shared_borrow(&contract_sig, i)
                            && !coerced.starts_with('&')
                            && !coerced.starts_with("&mut ")
                            && !crate::codegen::rust::call_site_borrow::expression_is_string_literal(
                                arg_to_generate,
                        ...
                        // regression-060: Copy-aggregate args must not keep a re-added `&` when the
                        // callee did not confirm shared-ref emission (`other: Lsn`).
                        if let Expression::Identifier { name, .. } = arg_to_generate {
                            let caller_copy_aggregate =
                                self.current_function_params.iter().any(|p| {
                                    p.name == *name
                                        && self.is_type_copy(&p.type_)
                                        && !crate::type_classification::is_copy_pass_by_value_formal(
                            ...
                            let callee_copy_aggregate = crate::codegen::rust::signature_promotion::emitted_owned_arg_contract(
                                &contract_sig, pidx,
                            ) || contract_sig
                                .formal_param_type(pidx)
                                .or_else(|| contract_sig.param_types.get(pidx))
                        ...
                        if matches!(
                            ...
                            if coerced.ends_with(".to_string()") {
                                coerced = coerced[..coerced.len() - 12].to_string();
                            }
                        ...
                        if let Expression::Identifier { name, .. } = arg_to_generate {
                            if self.match_arm_bindings.contains(name.as_str()) {
                                let pidx = contract_sig.arg_param_index(i);
                                let readonly_text = contract_sig
                                    .formal_param_type(pidx)
                                    .is_some_and(|t| {
                                        crate::codegen::rust::types::is_windjammer_text_type(t)
                                    })
                                    || contract_sig.param_types.get(pidx).is_some_and(|t| {
                        ...
                        // Final peel: Copy-aggregate caller into owned Copy-aggregate callee
                        // (`through: Lsn` → `other: Lsn`) must not keep stale `&` (regression-060).
                        if let Expression::Identifier { name, .. } = arg_to_generate {
                ...
                let sig_for_str_strip = call_site_sig.as_ref().or(method_signature.as_ref());
                if let Some(ref sig) = sig_for_str_strip {
                    let sig_param_idx = sig.arg_param_index(i);
                    if sig.param_types.get(sig_param_idx).is_some() {
                        let param_is_str_slice_ref = sig.param_types.get(sig_param_idx).is_some_and(
                            crate::codegen::rust::string_utilities::param_is_rust_str_ref,
                        ) || crate::codegen::rust::call_site_borrow::callee_emits_shared_rust_ref_param(
                            sig, sig_param_idx,
                        ) || (sig.formal_param_type(sig_param_idx).is_some_and(|t| {
                ...
                let string_literal_converted = if is_string_literal {
                    ...
                    if asref_str_module {
                        ...
                        let method_takes_borrowed_arg = effective_sig
                            .as_ref()
                            .and_then(|sig| sig.param_type_for_arg(i))
                            .is_some_and(|t| {
                                crate::codegen::rust::string_utilities::param_is_rust_str_ref(t)
                ...
                if is_string_literal
                    ...
                    && method_signature.as_ref().is_some_and(|sig| {
                        ...
                        !crate::codegen::rust::stdlib_method_traits::method_arg_expects_rust_str_ref_from_sig(sig, i)
                            && !crate::codegen::rust::call_site_borrow::callee_emits_shared_rust_ref_param(sig, pi)
                ...
                if !self.current_func_is_pure_forwarding_delegate
                    ...
                    if is_ref_to_copy && !arg_str.starts_with('*') {
                        ...
                        if base.contains('[') && base.contains("..") {
                            let param_wants_owned = method_signature
                                .as_ref()
                                .or(call_site_sig.as_ref())
                                .and_then(|sig| sig.param_type_for_arg(i))
                                .is_some_and(crate::codegen::rust::string_utilities::param_is_owned_string_type);
                            if param_wants_owned {
                                arg_str = format!("({}).to_string()", base);
                            } else {
                                crate::codegen::rust::expression_utilities::strip_trailing_clone(
                                    &mut arg_str,
                                );
                    ...
                    )
                {
                    ...
                    let expects_borrowed_ref = call_site_sig.as_ref().is_some_and(|sig| {
                        crate::codegen::rust::stdlib_method_traits::method_arg_expects_borrowed_reference_from_sig(
                            sig, i,
                        )
                    }) || crate::codegen::rust::stdlib_method_traits::method_arg_expects_borrowed_reference_qualified(
                    ...
                    if expects_str_ref || expects_borrowed_ref {
                        borrow_decision.add_ref = true;
                    }
                }
                ...
                let arg_expects_borrowed_ref = call_site_sig.as_ref().is_some_and(|sig| {
                    crate::codegen::rust::stdlib_method_traits::method_arg_expects_borrowed_reference_from_sig(
                        sig, i,
                    )
                }) || crate::codegen::rust::stdlib_method_traits::method_arg_expects_borrowed_reference_qualified(
                    ...
                    if is_ref_binding && !is_copy {
                        if let Some(ref sig) = sig_for_effective {
                            ...
                            if param_is_owned
                                ...
                                && !arg_str.ends_with(".clone()")
                            {
                                let skip_clone_for_map_key = is_collection_key_arg
                                    && matches!(
                                        arg_to_generate,
                                        Expression::Identifier { name, .. }
                                            if self.borrowed_iterator_vars.contains(name)
                                    );
                                if skip_clone_for_map_key {
                                    // Iterator keys are already `&K` — never clone for lookups.
                                } else {
                                let base = arg_str
                ...
                if external_module_mut_reborrow {
                    ...
                    let is_collection_key_arg = crate::codegen::rust::stdlib_method_traits::is_collection_key_lookup(
                        sig,
                        i,
                        receiver_type_name,
                    );
                    if param_is_mut_borrowed
                        && !param_wants_owned_value
                        && !is_collection_key_arg
                ...
                {
                    if let Expression::Cast { expr, type_, .. } = arg_to_generate {
                        let cast_target_copy =
                            crate::codegen::rust::type_analysis_pure::is_copy_type(type_);
                        if cast_target_copy {
                            // Copy cast keys (`key as usize`) pass by value — no `&(*key as T)`.
                        } else {
                            let inner = arg_str.trim();
                            let borrowee = if inner.starts_with('*') && inner.contains(" as ") {
                                format!("({})", inner.trim_start_matches('*'))
                            } else {
                                arg_str.clone()
                            };
                            arg_str = format!("&({borrowee})");
                        }
                    } else {
                ...
                if !arg_str.starts_with('&') {
                        ...
                        .or(method_signature.as_ref())
                    {
                        ...
                        if self.ir_sig_arg_expects_shared_borrow(sig, i)
                ...
                let wants_ref = legacy_sig.is_some_and(|sig| {
                    let expects_shared = crate::ir::signature_bridge::call_site_expects_shared_borrow(
                        sig, legacy_pidx,
                    );
                    let emitted_owned =
                        crate::codegen::rust::signature_promotion::emitted_owned_arg_contract(
                            sig, legacy_pidx,
                        ) || sig.formal_param_type(legacy_pidx).is_some_and(|t| {
                            let bare = match t {
                                Type::Reference(inner) | Type::MutableReference(inner) => {
                                    inner.as_ref()
                                }
                    ...
                    // Owned Copy aggregates beat stale analyzer/IR Ref (regression-060 Lsn).
                    expects_shared && !emitted_owned
                });
                ...
                if let Expression::Identifier { name, .. } = arg_to_generate {
                        ...
                        )
                    {
                        ...
                        {
                            let base = arg_str.trim_start_matches('&');
                            arg_str = if base.contains('[') && base.contains("..") {
                                format!("({base}).to_string()")
                            } else if base.ends_with(".clone()") {
                                base.to_string()
                            } else {
                                format!("{base}.clone()")
                            };
                ...
                if (self.callee_call_uses_rust_auto_borrow_for_owned_struct(arg_to_generate)
                    || legacy_sig.is_some_and(|sig| {
                        crate::codegen::rust::signature_promotion::emitted_owned_arg_contract(
                            ...
                            .formal_param_type(legacy_pidx)
                            .or_else(|| sig.param_types.get(legacy_pidx))
                            .is_some_and(|t| {
                            let bare = match t {
                                Type::Reference(inner) | Type::MutableReference(inner) => {
                                    inner.as_ref()
                ...
                // regression-060: final peel for Copy-aggregate caller → owned Copy-aggregate callee.
                if let Expression::Identifier { name, .. } = arg_to_generate {
                    let caller_copy = self.current_function_params.iter().any(|p| {
                        p.name == *name
                            && self.is_type_copy(&p.type_)
                    ...
                    let callee_copy = receiver_rt.and_then(|rt| {
                        self.resolve_method_function_signature(rt, method, arguments.len())
                    }).is_some_and(|sig| {
                        let cpidx = sig.arg_param_index(i);
                        crate::codegen::rust::signature_promotion::emitted_owned_arg_contract(
                            &sig, cpidx,
                        ) || sig
                            .formal_param_type(cpidx)
                            .or_else(|| sig.param_types.get(cpidx))
                            .is_some_and(|t| {
                                let bare = match t {
                                    Type::Reference(inner) | Type::MutableReference(inner) => {
                                        inner.as_ref()
