...
impl<'ast> CodeGenerator<'ast> {
    ...
    /// Build a [`FunctionSignature`] for formal parameter emission matching call-site `effective_param_ownership`.
    pub(in crate::codegen::rust) fn build_formal_emission_signature(
        &self,
        func: &FunctionDecl<'ast>,
        analyzed: &AnalyzedFunction<'ast>,
        ...
        let qualified = self
            .current_struct_name
            .as_ref()
            .map(|t| format!("{t}::{}", func.name));
        let registry_sig = qualified
            .as_ref()
            .and_then(|q| self.get_signature_with_global(q));
        ...
        for (idx, param) in func.parameters.iter().enumerate() {
            ...
            param_ownership.push(
                registry_sig
                    .and_then(|s| s.param_ownership.get(idx).copied())
                    .or_else(|| analyzed.inferred_ownership.get(&param.name).copied())
        ...
        if let Some(reg) = registry_sig {
            if !reg.param_ownership.is_empty() {
                param_ownership = reg.param_ownership.clone();
            }
            if !reg.formal_param_types.is_empty() {
                formal_param_types = reg.formal_param_types.clone();
            }
            // Keep bare `param_types` from declaration — registry may wrap Reference for
            // call-site metadata; formal/call effective ownership uses bare + formal_param_types.
        }
        ...
        FunctionSignature {
            ...
            has_self_receiver: func.parameters.iter().any(|p| p.name == "self"),
