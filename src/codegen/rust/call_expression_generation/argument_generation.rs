...
use super::super::{expression_helpers, expression_utilities, CodeGenerator};
            ...
            if let Some(&ownership) = sig.param_ownership.get(sig_param_idx) {
                match ownership {
                    OwnershipMode::Borrowed => {
                        ...
                        arg_str =
                            gen.ensure_ref_for_owned_string_field_when_callee_expects_str(
                                method_signature,
                                sig_param_idx,
                                arg_to_generate,
                                arg_str,
...
// --- Plain function call argument lowering ---
#[allow(clippy::too_many_lines)]
                        ...
                        OwnershipMode::MutBorrowed if !has_ownership_collision => {
                            ...
                            let is_already_mut_ref =
                                if let Expression::Identifier { name, .. } = arg {
                                    ...
                                    let inferred_mut_ref = gen.inferred_mut_borrowed_params.contains(name.as_str());
                                    explicit_mut_ref || inferred_mut_ref
                                } else {
                                    false
                                };
                            ...
                            if let Expression::FieldAccess { .. } = arg {
                                ...
                                if let Some(name) = root_name {
                                    ...
                                    if (is_borrowed_iterator_var
                                        ...
                                        && !arg_str.ends_with(".clone()")
                                    {
                                        let is_copy = gen.infer_expression_type(arg)
                                            .as_ref()
                                            .is_some_and(|t| gen.is_type_copy(t));
                                        if !is_copy {
                                            arg_str = format!("{}.clone()", arg_str);
